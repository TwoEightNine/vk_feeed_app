...
class SuggestionsFeedViewModel(repo: FeedRepository) : FeedViewModel(repo) {

    private var nextFrom: String? = null

    override fun loadFeed() {
        repo.loadSuggestions(nextFrom, ::onFeedLoaded, ::onErrorOccurred)
    }
    ...
    private fun onFeedLoaded(
        ...
        if (this.nextFrom.isNullOrEmpty()) {
            feedLiveData.value = feedLiveData.value
        }
    }
}